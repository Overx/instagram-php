...
/**
 * Nextpost Private API for Instagram v.4.0.5 with iOS emulation (iOS + Android)
 * 
 * @version NEXTPOST
...
class Instagram implements ExperimentsInterface
{
        ...
        $password) 
    {
        ...
        $this->_sendLoginFlow(false, 1800);

        $constants = $this->getDataFromWeb();
        $rollout_hash = isset($constants["rollout_hash"]) ? (string)$constants["rollout_hash"] : null;
        $query_hash_gr = isset($constants["query_hash_gr"]) ? (string)$constants["query_hash_gr"] : null; // query_hash for getReelsMediaFeedGraph() function
        ...
        $query_hash_gh = isset($constants["query_hash_gh"]) ? (string)$constants["query_hash_gh"] : null; // query_hash for getFeedGraph() function for hashtag
        $query_hash_gl = isset($constants["query_hash_gl"]) ? (string)$constants["query_hash_gl"] : null; // query_hash for getFeedGraph() function for location
        ...
        if ($query_hash_gr) {
            $this->settings->set('query_hash_gr', $query_hash_gr);
        } else {
            throw new \InstagramAPI\Exception\InstagramException("Couldn't detect query_hash_gr in loginGraph() function.");
        }

        if ($query_hash_gfs) {
            $this->settings->set('query_hash_gfs', $query_hash_gfs);
        } else {
            throw new \InstagramAPI\Exception\InstagramException("Couldn't detect query_hash_gfs in loginGraph() function.");
        }

        if ($query_hash_gfg) {
            $this->settings->set('query_hash_gfg', $query_hash_gfg);
        } else {
            throw new \InstagramAPI\Exception\InstagramException("Couldn't detect query_hash_gfg in loginGraph() function.");
        }

        if ($query_hash_gh) {
            $this->settings->set('query_hash_gh', $query_hash_gh);
        } else {
            throw new \InstagramAPI\Exception\InstagramException("Couldn't detect query_hash_gh in loginGraph() function.");
        }

        if ($query_hash_gl) {
            $this->settings->set('query_hash_gl', $query_hash_gl);
        } else {
            throw new \InstagramAPI\Exception\InstagramException("Couldn't detect query_hash_gl in loginGraph() function.");
        }

        if ($query_hash_guf) {
            $this->settings->set('query_hash_guf', $query_hash_guf);
        } else {
            throw new \InstagramAPI\Exception\InstagramException("Couldn't detect query_hash_guf in loginGraph() function.");
        }

        if ($query_hash_glk) {
            $this->settings->set('query_hash_glk', $query_hash_glk);
        } else {
            throw new \InstagramAPI\Exception\InstagramException("Couldn't detect query_hash_glk in loginGraph() function.");
        }
        ...
        $appRefreshInterval = 1800)
    {
        ...
        if ($justLoggedIn) {
            ...
            try {
                ...
                $this->direct->getBadgeCount();
                $this->internal->getNotificationBadge();
    ...
    public function getDataFromWeb() {
        ...
        if (isset($result[0][0])) {
            ...
            preg_match_all('/50,[a-zA-Z]="([a-zA-Z0-9]{32})",/', $response, $query_hashes);
            if (isset($query_hashes[1][0])) {
                $data["query_hash_gr"] = $query_hashes[1][0];
            }
            // Get query_hash for getFollowersGraph()
            preg_match_all('/[a-zA-Z]="([a-zA-Z0-9]{32})",[a-zA-Z]="([a-zA-Z0-9]{32})",[a-zA-Z]=1,[a-zA-Z]={inbound/', $response, $query_hashes);
            if (isset($query_hashes[1][0])) {
            ...
            preg_match_all('/pagination},queryId:"([a-zA-Z0-9]{32})"/', $response, $query_hashes);
            if (isset($query_hashes[1][0])) {
                $data["query_hash_guf"] = $query_hashes[1][0];
            }
            // Get query_hash for getLikersGraph()
            preg_match_all('/const [a-zA-Z]="([a-zA-Z0-9]{32})",[a-zA-Z]=1,[a-zA-Z]=/', $response, $query_hashes);
            if (isset($query_hashes[1][0])) {
                $data["query_hash_glk"] = $query_hashes[1][0];
            }
        }

        // Get query_hash for getFeedGraph() for hashtag
        $request = $this->request('explore/tags/apple/')
            ->setVersion(5)
            ->setAddDefaultHeaders(false)
            ->setSignedPost(false);
        if ($this->getIsAndroid()) {
        ...
        $response = $request->getRawResponse();
        preg_match_all('/(static\/bundles\/.+\/TagPageContainer\.js\/.+\.js)/', $response, $result);
        if (isset($result[0][0])) {
            $request = $this->request($result[0][0])
                ->setVersion(5)
                ->setAddDefaultHeaders(false)
            ...
            $response = $request->getRawResponse();
            preg_match_all('/queryId:"([a-zA-Z0-9]{32})"/', $response, $query_hashes);
            if (isset($query_hashes[1][0])) {
                $data["query_hash_gh"] = $query_hashes[1][0];
            }
        }

        // Get query_hash for getFeedGraph() for location
        $request = $this->request('explore/locations/397152560821930/')
            ->setVersion(5)
            ->setAddDefaultHeaders(false)
            ->setSignedPost(false);
        if ($this->getIsAndroid()) {
        ...
        $response = $request->getRawResponse();
        preg_match_all('/(static\/bundles\/.+\/LocationPageContainer\.js\/.+\.js)/', $response, $result);
        if (isset($result[0][0])) {
            $request = $this->request($result[0][0])
                ->setVersion(5)
                ->setAddDefaultHeaders(false)
            ...
            preg_match_all('/queryId:"([a-zA-Z0-9]{32})"/', $response, $query_hashes);
            if (isset($query_hashes[1][0])) {
                $data["query_hash_gl"] = $query_hashes[1][0];
            }
        }
